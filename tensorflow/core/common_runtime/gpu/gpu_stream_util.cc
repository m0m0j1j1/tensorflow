...
bool ReadRuleFromFile(
  ...
  if (!stream_from_file.empty()) {
    ...
    if (file.is_open()) {
      ...
      VLOG(0) << "Read stream assignment rule from file failed: "
              << stream_from_file << ". Use the default rule...";
      return false;
    }
  } else {
    VLOG(2) << "No stream assignment rule file specified! Use the default "
               "rule...";
    return false;
  }
  VLOG(0) << "Successfully read stream assignment rule from file: "
          << stream_from_file;
  return true;
}
...
void GetStreamWaitList(
  ...
  for (Node* n : graph->nodes()) {
    ...
    if (stream_id == 0) {
      // Never wait for the nodes on stream 0.
      continue;
    }
...
Status AssignStreams(const Graph* graph, const AssignStreamsOpts& opts,
  ...
  for (Node* n : order) {
    ...
    if (has_rule) {
      ...
      if (rule_stream_id != 0) stream_id = rule_stream_id;
    }
  ...
  // Reverse propagation for Recv on stream 0.
  for (auto it = order.rbegin(); it < order.rend(); ++it) {
    Node* n = *it;
    const int node_id = n->id();
    if (IsRecv(n) && (*node_to_stream_id)[node_id] == 0) {
      int stream_id = 0;
      for (const Edge* e : n->out_edges()) {
        int tmp = (*node_to_stream_id)[e->dst()->id()];
        if (tmp > 0) {
          stream_id = tmp;
          break;
        }
      ...
      if (ms_key_info_log)
        VLOG(0) << " Recv switch form stream 0 to " << stream_id;
      (*node_to_stream_id)[node_id] = stream_id;
    }
  }
