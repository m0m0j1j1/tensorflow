...
void adv_tokenizer(string s, char del, std::vector<string> &items) {
...
void GenerateInputsWDL(const GraphDef &graph_def,
                       const std::vector<string> &input_names,
                       std::vector<Tensor> &input_tensors, int batch_size) {
  // create tensors
  std::unordered_map<string, Tensor> f_dict;
  for (int i = 1; i < 14; ++i) {
    string key = "I" + std::to_string(i);
    TensorShape tensorShape;
    tensorShape.AddDim(batch_size);
    f_dict[key] = Tensor(DT_FLOAT, tensorShape);
  }
  for (int i = 14; i < 40; ++i) {
    string key = "C" + std::to_string(i - 13);
    TensorShape tensorShape;
    tensorShape.AddDim(batch_size);
    f_dict[key] = Tensor(DT_STRING, tensorShape);
  }

  // fill data
  std::ifstream read_file;
  read_file.open(data_path, std::ios::binary);
  for (int l = 0; l < batch_size; ++l) {
    string line;
    getline(read_file, line);
    std::vector<string> items;
    adv_tokenizer(line, ',', items);
    for (int i = 1; i < 14; ++i) {
      string key = "I" + std::to_string(i);
      auto &t = f_dict[key];
      float *data = t.flat<float>().data();
      data[l] = std::stof(items[i]);
    }
    for (int i = 14; i < 40; ++i) {
      string key = "C" + std::to_string(i - 13);
      auto &t = f_dict[key];
      tstring *data = t.flat<tstring>().data();
      data[l] = tstring(items[i]);
    }
  }

  // get output
  for (auto &name : input_names) {
    input_tensors.push_back(std::move(f_dict[name]));
  }
}
...
Status Test(SavedModelBundle &bundle, std::vector<std::string> &input_names,
            std::vector<std::string> &output_names, int batch_size,
            int num_infers_per_thread, int num_threads) {
  const GraphDef &graph_def = bundle.meta_graph_def.graph_def();
  std::unique_ptr<Session> session(std::move(bundle.session));
  // TF_CHECK_OK(WriteTextProto(Env::Default(),
  // "/home/robinz/tensorflow/tensorflow/cc/tutorials/resnet_50/1/model.pbtxt",
  // graph_def));
  ...
  GenerateInputsWDL(graph_def, input_names, input_tensors_tf, batch_size);
...
int main(int argc, char *argv[]) {
  ...
  tensorflow::example::SetSessionOptions(options, config_proto);
  status = LoadSavedModel(options, RunOptions(), model_path,
                          std::unordered_set<std::string>{"serve"}, &bundle);
  ...
  TF_CHECK_OK(example::Test(bundle, input_names, output_names, batch_size,
                            num_infers_per_thread, num_threads));
