...
bool ParseRunmeta(const std::vector<string> &input_names,
  ...
  for (auto &name : input_names) {
    string file_name;
    if (name == "rtp/fg/fill_attributes/qinfo/user_slot_pos_1/1") {
      file_name = "rtp/fg/fill_attributes/qinfo/user_slot_pos_1:1";
    } else {
      file_name = name + ":0";
    }
...
void GenerateInputsKGB(const GraphDef &graph_def,
                       const std::vector<string> &input_names,
                       std::vector<Tensor> &input_tensors) {
...
Status Test(GraphDef &graph_def, std::vector<std::string> &input_names,
  ...
  GenerateInputsKGB(graph_def, input_names, input_tensors_tf);
